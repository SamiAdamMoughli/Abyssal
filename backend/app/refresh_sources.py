...
SOURCES = [iuu_list, sanctions, port_control, eez]
...
def main() -> None:
    ...
    for mod in SOURCES:
        ...
        try:
            result = mod.refresh()
            after = cache_info(mod.SOURCE)
            print(f"  [OK ] {mod.SOURCE:14s} vorher: {age:>12s} | "
                  f"jetzt: frisch | Eintraege: {list(result.values())[-1]}")
